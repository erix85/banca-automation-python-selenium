...
from src.actions.web.autenticacion.login.login_actions import LoginActions as WebLoginActions

from src.actions.api.login.api_login_actions import LoginActions as ApiLoginActions
...
def before_all(context):
    ...
    # 'vivienda' es la raíz del proyecto, así que navegamos hasta ella.

    context.project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))  # Esto te lleva a 'vivienda'
...
def before_scenario(context, scenario):
    ...
    if 'web' in scenario.tags:
        ...
        try:
            ...
            if context.driver:  # Asegúrate de que el driver se haya creado

                context.driver.maximize_window()

                context.driver.get(context.base_url)  # Navega a la URL base

                # Set implicit wait after driver initialization

                context.driver.implicitly_wait(context.implicit_wait)  # Usa el valor ya parseado de context.config_env

                context.logger.info(f"URL Base Web: {context.base_url}")

                context.logger.info(f"Navegador usado: {context.browser_name}")

                context.selenium_utils = SeleniumUtils(context.driver,
                                                       context.wait_timeout)  # Pasa wait_timeout si SeleniumUtils lo necesita

                context.login_actions = WebLoginActions(context.driver, context.selenium_utils)
            ...
            context.logger.critical(f"Error inesperado al inicializar WebDriver para {context.browser_name}: {e}")

            scenario.skip(f"Fallo al inicializar WebDriver: {e}")  # Usar scenario.skip para marcar como saltado
